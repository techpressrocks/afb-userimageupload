...
*/
    ...
    function post_form( $post_type ) {
        ...
        <div id="auiu-post-area">
			<form id="auiu_new_post_form" name="auiu_new_post_form" action="" enctype="multipart/form-data" method="POST">
                ...
                <?php if ( auiu_get_option( 'allow_cats', 'auiu_frontend_posting', 'on' ) == 'on' ) { ?>
                    <li>
                        <label for="new-post-cat">
                            <?php echo auiu_get_option( 'cat_label', 'auiu_labels', 'Category' ); ?>
                        </label>
                        <div class="category-wrap" style="float:left;">
						     <div id="lvl0">
                                ...
                                if ( $cat_type == 'normal' ) {
                                    wp_dropdown_categories( '&taxonomy='.$default_taxonomy.'&hierarchical=1&hide_empty=0&orderby=name&name=category[]&id=cat&show_count=0&title_li=&use_desc_for_title=1&class=cat&selected=1&exclude=' . $exclude );
                                } else if ( $cat_type == 'ajax' ) {
									wp_dropdown_categories( '&taxonomy='.$default_taxonomy.'&hierarchical=1&hide_empty=0&orderby=name&name=category[]&id=cat-ajax&show_count=0&title_li=&use_desc_for_title=1&class=cat&selected=1&depth=1&exclude=' . $exclude );
                                } else {
						...
						<span class="description"><?php echo stripslashes( auiu_get_option( 'cat_help', 'auiu_labels' ) ); ?></span>
				...
				<li>
					<label for="cf_agreed">
                    Yes I Agree<span class="required">*</span>
                    </label>
                    <div class="auiu-check-container">
                        <input type="hidden" name="cf_agreed" value="" />
						<input type="checkbox" id="auiu-checkbox" class="requiredField"  name="cf_agreed[]" value="Yes" />
						<span>Yes</span>
                    </div>
                    <div class="clear"></div>
                    <p class="description"><?php echo stripslashes( auiu_get_option( 'agree_text', 'auiu_enablecustom' ) ); ?>
					<a href="<?php echo stripslashes( auiu_get_option( 'terms_link', 'auiu_enablecustom' ) ); ?>"><?php echo stripslashes( auiu_get_option( 'terms_link', 'auiu_enablecustom' ) ); ?></a>
	...
	$errors = apply_filters( 'auiu_add_post_validation', $errors );
		...
		$post_author = auiu_get_option( 'map_author', 'auiu_frontend_posting' );
